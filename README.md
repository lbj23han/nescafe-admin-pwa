...
- Next.js App Router + Server Component 기반 인증 흐름
- 쿠키 기반 세션 유지로 SSR 환경 안정화
- `/auth/callback`을 통한 OAuth/Email 인증 처리

---

### 👤 계정 & 권한 모델

- 계정 단위 데이터 접근 (Shop 단위 분리)
- `profiles` 테이블 기반 권한 관리
- 권한 레벨
  - **Owner**: 전체 관리 권한
  - **Admin**: 관리 권한
  - **Staff / Viewer**: 실무 접근 (읽기/입력)
...
- 초대 수락 시
  - `profiles.shop_id`, `profiles.role` 자동 연결
...
> 직책/권한/가게명 모두 **백엔드 단일 소스 기반**으로 표시  
> (user_metadata 의존 제거)
